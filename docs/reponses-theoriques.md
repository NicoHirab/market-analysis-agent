...
- verdicts du juge par critère et taux de révision — déjà calculés par analyse (`judge_passed`, `judge_criteria`, `revised` dans les métadonnées de réponse) ; il manque seulement l'agrégation dans le temps (taux de passage par critère) ;
...
**Alerting** : taux d'erreurs `LLM_FAILURE` au-dessus d'un seuil, coût cumulé/jour au-dessus d'un budget, taux d'échec d'un critère du juge en hausse sur une fenêtre glissante — signal de dérive qualité, la donnée brute existe déjà par analyse, il manque l'agrégation et le seuil —, latence p95 en hausse.

**Qualité des sorties.** Le nœud `judge` est déjà, de facto, une évaluation automatique par rapport, critère par critère (ancrage, complétude, actionnabilité — acceptation seulement si tous passent) — l'étape 7 détaille comment l'étendre en évaluation offline. En complément, un échantillonnage humain périodique (relire N rapports/semaine tirés au hasard avec la même grille que le juge) sert à calibrer la dérive entre les verdicts du juge et un jugement humain — sans quoi une dérive du juge lui-même passerait inaperçue.
...
**Le LLM-as-judge est déjà en production dans ce dépôt**, pas seulement à l'état de proposition théorique : le nœud `judge` (`agent/nodes.py`) évalue chaque rapport **critère par critère** — ancrage dans les données, complétude par rapport au plan, actionnabilité — avec un verdict pass/fail par critère ; le rapport n'est accepté que si tous passent (conjonction appliquée en code, jamais déléguée au modèle), et un échec déclenche une révision bornée (`graph.py`). L'étendre en évaluation offline est direct puisque c'est déjà une fonction pure de `(plan, données sources, rapport)` — le juge reçoit les mêmes données que la synthèse, l'ancrage est donc vérifiable et non déclaratif — réutilisable hors du chemin de requête live : geler un golden set de requêtes avec le comportement attendu (quel plan, quels faits doivent apparaître), le rejouer à chaque changement de prompt ou de modèle, et suivre le taux de passage par critère dans le temps comme signal de non-régression.

**Comparaison de prompts.** Remplacer les constantes de prompt codées en dur dans `agent/nodes.py` par le registre versionné `agent_configs` introduit à l'étape 4 (nom, version, `prompt_template`/seuils, `is_active`). L'A/B se fait par assignation aléatoire — mais déterministe par hash de l'identifiant d'analyse, pour qu'un run rejoué reste reproductible — d'une variante par analyse, avec comme métriques de comparaison le taux de passage des critères du juge et, une fois branché, le feedback utilisateur ci-dessous.
