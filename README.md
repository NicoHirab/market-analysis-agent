...
- **planner** *(LLM)* — interprète la requête et produit un `AnalysisPlan` structuré `{analyses, platforms, rationale}`. C'est le point de décision agentique du système : « compare les prix du PS5 » ne déclenche pas d'analyse de sentiment, « pourquoi les clients sont déçus » la déclenche. Si la requête HTTP impose déjà les analyses ou les plateformes, le planner les traite comme une contrainte dure et ne planifie que le reste.
...
|---|---|---|
| `api` | `src/market_agent/api/` | FastAPI : routes REST + SSE, schémas de requête/réponse, registre de jobs en mémoire |
...
Un processus unique par instance : chaque requête HTTP crée un `AnalysisState`, exécute le graphe compilé de façon asynchrone, publie les événements de progression nœud par nœud, et stocke le résultat dans un registre en mémoire — limite assumée du MVP, détaillée à l'étape 4.
...
`demo.sh` enchaîne un health-check, soumet une analyse, affiche le flux SSE en direct nœud par nœud, puis imprime le rapport Markdown et les métadonnées (coût, tokens, score du juge). Le fournisseur par défaut (`LLM_PROVIDER=mock`, voir `docker-compose.yml`) est un simulateur déterministe : aucune clé API, aucun appel réseau sortant, résultat reproductible pour n'importe quelle requête.
...
| `LLM_PRICE_IN_PER_MTOK` / `LLM_PRICE_OUT_PER_MTOK` | *(vide)* | si les deux sont renseignées, active `cost_estimate_usd` dans la réponse (local uniquement — voir Limites) |
...
**Une orchestration native** (boucle `asyncio` + appels LLM directs, sans framework) aurait été la démonstration la plus « from scratch » de maîtrise des fondamentaux. Mais son coût réel — ré-implémenter proprement le fan-out parallèle, le suivi d'état entre nœuds, le streaming incrémental, une boucle de révision bornée, la reprise sur erreur — n'apporte aucune valeur différenciante pour ce test : ce n'est pas là-dessus que se juge la qualité de la réflexion, et le risque de bugs de plomberie (état partagé, races) est réel. LangGraph fournit ces briques nativement (`StateGraph`, edges conditionnels pouvant retourner plusieurs branches à la fois, `astream(stream_mode=["updates", "values"])`) et laisse le budget disponible pour ce qui compte réellement : la conception du graphe et sa justification écrite.
...
**LangGraph** a donc été retenu : contrôle explicite du graphe (fan-out parallèle, boucle de révision bornée par construction, streaming nœud par nœud natif) sans réécrire un runtime d'orchestration, et une abstraction de modèle (via `langchain-core`) qui ne verrouille aucun fournisseur.
...
| `GET /api/v1/analyses/{id}` | Statut + résultat complet : rapport, plan, verdict du juge, erreurs, métadonnées (`provider`, `model`, `duration_ms`, `llm_calls`, tokens, `cost_estimate_usd`, `judge_score`, `revised`, `degraded`). |
| `GET /api/v1/analyses/{id}/events` | Flux SSE de progression : `analysis_started`, un `node_completed` par nœud franchi, `analysis_completed` en clôture. Rejoue le snapshot connu puis bascule sur le flux live. |
...
**3. Suivi en direct (SSE)**

```bash
curl -sN "http://localhost:8000/api/v1/analyses/$ID/events"
```
...
65 tests, tous hors-ligne et déterministes — aucun appel réseau, aucun coût, environ 2 secondes d'exécution totale. Le seam `StructuredLLM` (provider `mock`) alimente tout appel LLM pendant les tests exactement comme il alimente la démo zéro-clé ; les adaptateurs `scraper` mockés jouent le même rôle côté données. `pytest-asyncio` tourne en mode `auto`.

| Fichier | Tests | Couvre |
|---|---|---|
| `test_nodes.py` | 11 | Chaque nœud à l'unité — respect des contraintes du planner, résilience partielle/totale de `collect`, dégradation `sentiment`/`trends`/`judge` |
| `test_llm.py` | 10 | Factory par fournisseur (mock/anthropic/openai-compatible), sémantique du mock, retry correctif LangChain |
| `test_api.py` | 10 | `?wait=true`, async + poll, 422/404, run dégradé sur plateforme inconnue, ordre des événements SSE (snapshot + live), fail-fast au démarrage si `LLM_API_KEY` manquante |
| `test_registry.py` | 8 | Cycle de vie d'un job, publication/abonnement des événements |
...
- **`analysis_events`** — table append-only : `id`, `analysis_id` (fk), `type`, `node`, `payload` (jsonb), `created_at`. L'équivalent durable du buffer d'événements en mémoire du registre actuel (`JobRegistry.publish`) : elle sert à la fois de trace d'audit et de source de rejeu pour `/events` — un processus qui redémarre peut reconstruire le flux SSE depuis la base plutôt que depuis une mémoire perdue.
...
- *Budgets par requête* : `cost_estimate_usd` est déjà calculé après coup (`_build_meta`) ; il manque une vérification avant/pendant l'exécution pour vraiment plafonner, pas seulement rapporter.
...
- **SSE sans reprise par identifiant d'événement** — un client qui se déconnecte puis se reconnecte revoit le snapshot connu à cet instant, pas un rejeu exact depuis un `Last-Event-ID` précis (l'historique en mémoire n'a pas d'identifiant monotone durable). Résolu naturellement par `analysis_events` (étape 4).
- **Pas de cache** — chaque requête, même identique à une précédente, rejoue tout le graphe (voir étape 6).
- **Estimation de coût optionnelle et manuelle** — `LLM_PRICE_IN_PER_MTOK`/`LLM_PRICE_OUT_PER_MTOK` doivent être renseignées à la main (aucune API de tarification consultée), et ne sont volontairement pas propagées par `docker-compose.yml` : une valeur vide casserait le parsing numérique (voir le commentaire dans `.env.example`).
...
Vérifié le 20 juillet 2026 sur la branche `feat/market-analysis-agent` : `make test` (65 tests), `make lint`, `docker compose up --build` et `bash scripts/demo.sh` exécutés avec succès avant commit.
